# Deploy BTExtract — Fly.io + Streamlit Cloud
...
GitHub (repositório)
    ...
    │                        https://btextract-bertoi.fly.dev
    │
...
2. Nome sugerido: `btextract`
3. Deixe **Privado** (seus dados de clientes ficam protegidos)
4. Clique em "Create repository"

### 1.2 Enviar o código

Abra o terminal na pasta `C:\BTExtract` e execute:

```bash
git init
git add .
git commit -m "deploy: BTExtract v2 — Fly.io + Streamlit Cloud"
git branch -M main
git remote add origin https://github.com/SEU_USUARIO/btextract.git
...
fly volumes create btextract_data --region gru --size 1
```
...
fly secrets set SMTP_FROM="BTExtract <seu@gmail.com>"
```
...
Visit your newly deployed app at https://btextract-bertoi.fly.dev
```
...
Acesse `https://btextract-bertoi.fly.dev/health` — deve retornar `{"status":"ok"}`.
...
Preencha os campos:
- **Repository:** `SEU_USUARIO/btextract`
- **Branch:** `main`
- **Main file path:** `dashboard.py`
- **App URL:** escolha um nome (ex: `btextract-bertoi`)

Clique em **"Advanced settings"** e em **Secrets** cole:

```toml
API_BASE_URL = "https://btextract-bertoi.fly.dev"
```
...
## PARTE 4 — Atualizar o link do Dashboard no BTExtract

Depois que o Streamlit Cloud gerar a URL definitiva, atualize o link no frontend.

Em `C:\BTExtract\templates\frontend.html`, localize a função `_initDashboardBtn`:
...
btn.href = isLocal
  ? `http://${window.location.hostname}:8081`
  : 'https://btextract-bertoi.streamlit.app';  // ← URL real do Streamlit
```
...
| Serviço | URL |
|---|---|
| BTExtract (FastAPI) | https://btextract-bertoi.fly.dev |
| Dashboard (Streamlit) | https://btextract-bertoi.streamlit.app |
| Health check | https://btextract-bertoi.fly.dev/health |
| Docs API | https://btextract-bertoi.fly.dev/docs |
...
# Reiniciar app
fly apps restart btextract-bertoi
...
fly ssh console
sqlite3 /data/btextract.db

# Fazer backup do banco
fly ssh sftp get /data/btextract.db ./backup_$(date +%Y%m%d).db
